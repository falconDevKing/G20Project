...
export const stripeCreatePaymentIntent = async (reqData: any) => {
  try {
    ...
    const successRes = successResponseCreator(200, "Payment Intent Created", {
    ...
    const errorRes = errorResponseCreator(400, err.message || "Payment Intent Unsuccessful", err);
    return errorRes;
  }
};

export const stripeCreateRecurringPaymentIntent = async (reqData: any) => {
  try {
    // extract vars
    const { user_id, user_name, user_email, user_address, phone } = reqData;

    let customer;

    // find or create customer
    const { data: customerData, error: findCustomerError } = await SupabaseClient.from("partner").select().eq("id", user_id).maybeSingle();

    if (customerData && !!customerData?.stripe_customer_id) {
      customer = {
        id: customerData.stripe_customer_id,
      };
      console.log("found user customer id");
    } else {
      ...
      const { data: updatedUser, error: updateUserError } = await SupabaseClient.from("partner").update(customerIdUpdate).eq("id", user_id).select();
      console.log("updated existing customer id");
    }
    ...
    console.log("created new setup intent", setupIntent);

    const successRes = successResponseCreator(200, "Setup Intent Created", {
...
export const stripeCreateRecurringPayment = async (reqData: any) => {
  try {
    // extract vars
    const { user_id, user_name, amount, currency, customer_id, payment_method_id, unique_code, remission_day, user_description, first_payment_day } = reqData;
    ...
    const price = await stripe.prices.create({
      ...
      product_data: {
        name: user_name + " monthly remission",
      },
    });
    ...
    const subscriptionCreationObject = {
      ...
      description: user_description || "Monthly Subscription for " + user_name,
      items: [
        {
          price: price.id,
        },
    ...
    console.log("created new subscription, ", subscription.id, subscription);
    ...
    const { data: updatedUser, error: updateUserError } = await SupabaseClient.from("partner").update(customerSubscriptionIdUpdate).eq("id", user_id).select();

    console.log("updatedUser", updatedUser);
    const successRes = successResponseCreator(200, "Payment Subscription Successful", {
      subscription,
      paidInvoice,
    });
    ...
    console.log("err", err.message, err);
    const errorRes = errorResponseCreator(400, err.message || "Recurring Payment Unsuccessful", err);
    return errorRes;
  }
};
