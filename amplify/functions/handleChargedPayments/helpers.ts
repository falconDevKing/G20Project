...
import { PartnerUpdateType, PaymentRowType, PaystactRecurringPaymentsUpdateType } from "../../interfaces/modifiedSupabaseTypes";
...
export const getPendingPayments = async (id: string) => {
  const { data, error } = await SupabaseClient.from("paystack_pending_payments").select().eq("id", id).maybeSingle();
...
export const makePayment = async (paymentData: PaymentRowType, log = false) => {
...
export const updateUserData = async (userId: string, userData: PartnerUpdateType) => {
  const { data, error } = await SupabaseClient.from("partner").update(userData).eq("id", userId).select().maybeSingle();
...
export const updateRecurringPaymentData = async (recurringId: string, recurringPaymentData: PaystactRecurringPaymentsUpdateType) => {
  const { data, error } = await SupabaseClient.from("paystack_recurring_payments").update(recurringPaymentData).eq("id", recurringId).select().maybeSingle();
...
export const deletePendingPayments = async (id: string) => {
  await SupabaseClient.from("paystack_pending_payments").delete().eq("id", id);
};
...
const findChapterDetails = async (chapterId: string) => {
...
export const handleMailingAndStatusUpdates = async ({
  ...
  user_id &&
    ...
    (await sendPaymentReceivedMessage({
      ...
      approved_by_name: "Online Monthly Paystack",
    }));

  await updateUserStatus(user_id === "Guest" ? "" : user_id, remission_start_date);
};
