...
import { PartnerPaymentDetails } from "@/components/PartnerDetails";
...
interface HoGUserDataTableProps<T> {
  data: T[];
  count: number;
...
export default function HoGUserDataTable<T>({
  data,
  columns,
  count,
  page,
  ...
  return (
    <div className="w-full  md:border border-[#ae9956] dark:bg-[#252525]/35 dark:border-[#EDEDED24] rounded-xl shadow-sm overflow-hidden">
      ...
      <Table>
        ...
        <MobileTableHeader tableType="hogUsers" />
          ...
          {table.getRowModel().rows.length > 0 ? (
            table.getRowModel().rows.map((row) =>
              isLargeScreen ? (
                ...
                <MobileTableCard key={row.id} row={row} tableType="hogUsers" openUpdateDialog={openUpdateDialog} />
              ),
            )
          ) : (
            <TableRow>
