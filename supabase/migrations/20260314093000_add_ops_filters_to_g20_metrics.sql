...
begin
  ...
  return jsonb_build_object(
    'Status', jsonb_build_array(
      jsonb_build_object('title', 'Total Honourables', 'value', total_partners),
      jsonb_build_object('title', 'Consistent Honourables', 'value', consistent_partners),
      jsonb_build_object('title', 'Active Honourables', 'value', active_partners),
      jsonb_build_object('title', 'Passive Honourables', 'value', passive_partners)
    ),
  ...
  period_totals as (
    select
      ...
      coalesce(sum(p.amount) filter (
        where coalesce(p.approved_by, '') ilike '%online%'
      )::numeric, 0) as online_value,
      coalesce(sum(p.amount) filter (
        where not coalesce(p.approved_by, '') ilike '%online%'
      )::numeric, 0) as offline_value,
      count(*) filter (where coalesce(p.approved_by, '') ilike '%online%') as online_count,
